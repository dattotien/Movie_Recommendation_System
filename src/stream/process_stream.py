...
def get_real_model():
    ...
    if global_als_model is None:
        print(f"--- [REAL MODEL] Đang tải mô hình ALS từ: {HDFS_MODEL_PATH} ---")
...
def create_process_batch(item_factors_map: Dict[int, np.ndarray]):
    """
    Trả về hàm process_batch có đóng gói item_factors_map.
    """
    ...
    def process_batch(batch_df, batch_id):
        ...
        for row in batch_records:
            ...
            if item_vec is None:
                print(f"⚠️ Không tìm thấy item factor cho movie {movie_id}.")
        ...
        for user_id, entries in user_events.items():
            ...
            try:
                ...
                if recs:
                    ...
                    online_updates += 1
                    print(f"✅ Đã cập nhật online cho user {user_id} (top {len(recs)}).")
                else:
                    print(f"⚠️ Không sinh được recs cho user {user_id}, fallback model.")
                    fallback_user_ids.add(user_id)
            except Exception as err:
                print(f"⚠️ Lỗi khi cập nhật online cho user {user_id}: {err}")
                fallback_user_ids.add(user_id)
        ...
        if fallback_user_ids:
            ...
            print(f"--- [REAL DB] Đã fallback {len(results)} users ---")
