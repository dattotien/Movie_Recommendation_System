...
# 2. Build lại image app và khởi động toàn bộ hệ thống
docker compose up -d

# 3. Kiểm tra trạng thái các container (bạn thấy tất cả đều "Up")
docker compose ps

# 4. Chui vào bên trong container 'app'
docker compose exec app bash 
# 5. Kiểm tra xem thư mục model có tồn tại không
ls src/batch/als_model_32m

# 6. Chạy script Lớp Batch (Huấn luyện/Tính toán)
# (Tên file này có vẻ là "train_model.py" trong kế hoạch, nhưng bạn chạy file "write_recommendations.py")
spark-submit src/batch/write_recommendations.py

# 7. Test đọc từ Cassandra (cho user 1) -> THÀNH CÔNG
python -c "from utils import cassandra_connector as cc; print(cc.read_recs(1))"

# 8. Chạy Web App (Lớp Serving)
python src/webapp/app.py

# 9. Test đọc từ Cassandra (cho user 5) -> THÀNH CÔNG
python -c "from utils import cassandra_connector as cc; print(cc.read_recs(5))"

# 10. Chạy lại Lớp Batch (có thể để test)
spark-submit src/batch/write_recommendations.py

# 11. Test đọc từ Cassandra (cho user 5) -> THÀNH CÔNG
python -c "from utils import cassandra_connector as cc; print(cc.read_recs(5))"
